...
def getContours(img,imgContour,door):
    ...
    if door.lock:
        ...
        if len(door.higher_box) > 0:
            ...
            cv.putText(imgContour,'High Score',(door.higher_box[1][0],door.higher_box[1][1]),cv.FONT_HERSHEY_DUPLEX,0.7,(0,0,255),2)
        if len(door.lower_box) > 0:
            cv.drawContours(imgContour, [door.lower_box], -1, (255, 0, 255), 7)
            cv.putText(imgContour, 'Low Score', (door.lower_box[1][0],door.lower_box[1][1]), cv.FONT_HERSHEY_DUPLEX, 0.7, (0, 0, 255), 2)
